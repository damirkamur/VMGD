...
# 1. ===================================== Input Data
A, B = 0, 1
Elem = np.array([2, 3, 4, 5, 6, 7, 8, 9, 10, 25])
Nt_elem = np.zeros(len(Elem))
Nt_max = np.zeros(len(Elem))
NtN2 = np.zeros(len(Elem))
...
plt.loglog(Nt_elem, Nt_max, Nt_elem, NtN2)
plt.legend(("N_max", "N_std"))
...
plt.ylabel('N2')
# plt.show()
plt.savefig(f'pictures/Невязка - loglog .png', dpi=300)
