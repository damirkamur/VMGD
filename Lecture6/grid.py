...
class Grid:
    def __init__(self, vert: np.ndarray, elem_vert: list):
        ...
        self.boundary_vert, self.internal_vert = gu_vert_types(self)
...
def gu_vert_types(grid: Grid) -> tuple[list, list]:
    boundary_vertexes = set()
    for iface in grid.boundary_faces:
        boundary_vertexes.add(grid.face_vert[iface][0])
        boundary_vertexes.add(grid.face_vert[iface][1])
    boundary_vertexes = list(boundary_vertexes)
    internal_vertexes = list()
    for i in range(grid.Nvert):
        if i not in boundary_vertexes:
            internal_vertexes.append(i)
    return boundary_vertexes, internal_vertexes
